(() => {
  ...
  function skeletonCards(n = PAGE_SIZE) {
    const col = () => `<div class="col">
      <article class="card h-100" aria-busy="true">
        <figure class="card__media mb-0"></figure>
      </article>
    </div>`;
  ...
  function emptyState(message, withRetry) {
    ...
    return `<div class="col">
      <div class="p-4 text-center border rounded-3 bg-white">
        ...
        ${cta ? `<div class="text-center mt-2">${cta}</div>` : ''}
      </div>
    </div>`;
  }
  ...
  function cardHTML(id) {
    ...
    return `<div class="col">
      <article class="card h-100">
        ...
        <button class="fav" type="button" data-id="${id}" aria-pressed="${active ? 'true' : 'false'}">${active ? '★' : '☆'}</button>
      </article>
    </div>`;
  }
  ...
  async function renderBrowse() {
    ...
    try {
      ...
      state.hasNext = ids.length === getPageSize();
      gridEl.innerHTML = ids.length
        ? ids.map(cardHTML).join('')
        : emptyState('No cats found, try again.', true);
    } catch {
      if (reqId !== state.reqId) return;
  ...
  function renderFavorites() {
    ...
    if (!ids.length) {
      ...
      nextBtn.hidden = true;
      updatePager();
      gridEl.innerHTML = `<div class="col">
        <div class="p-4 text-center border rounded-3 bg-white">
          <div class="mb-2" aria-hidden="true" style="font-size:40px">🐾</div>
